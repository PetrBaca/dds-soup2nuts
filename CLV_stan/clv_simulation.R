...
source("CLV_stan/theme_og.R")

# Read in and format a sample of the cdnow data set
custs <- read.table("CLV_stan/CDNOW_sample.txt") %>%
  select(-V1) %>%
...
# Fit model
model_fit <- stan(file = "CLV_stan/full_clv.stan",
                  data = model_data,
                  chains = 4,
                  iter = 1000)
...
exp_trans_dist <- data.frame(distro = c(extract(model_fit)$exp_trans),
                             customer_id = rev(factor(rep(c(1:98), each = 2000)))) %>%
